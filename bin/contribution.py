...
def format_issues(issues):
    ...
    issues_sorted = sorted(tqdm(islice(issues, MAX_PR)), key=lambda i: (i.repository.stargazers_count, i.repository.full_name, i.created_at), reverse=True)
    ...
    for i, (repo, issues) in enumerate(issues_by_repository):
        ...
        if repo.stargazers_count < MIN_STARGAZE:
            ...
            minimum_repo+=1
            continue
        star_img = f"![about {repo.stargazers_count} stars](https://img.shields.io/github/stars/{repo.full_name}?style=plastic)"
        fork_img = f"![about {repo.forks} forks](https://img.shields.io/github/forks/{repo.full_name}?style=plastic)"
        yield f"* [{repo.full_name}]({repo.html_url}) {star_img} {fork_img}"
        yield ""
        if issue_count > 100:
            yield " " * 4 + f"* skipped {issue_count} for the sake of clarity"
        else:
            for issue in issues:
                status = "[ ]" if issue.state == 'open' else "[x]"
                yield " " * 4 + f"* {status} [{issue.title}]({issue.html_url}) - {issue.created_at.strftime('%Y.%m.%d')}"
    yield ""
    yield f"Also, {minimum_issue} pull requests in {minimum_repo} repositories not worth mentioning (< {MIN_STARGAZE} stars) here for the sake of clarity"
